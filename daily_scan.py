...
def rank(rows) -> pd.DataFrame:
    ...
    for col, w in [("rs", 2.0), ("mom", 1.0), ("volsurge", 1.0)]:
        s = df[col]
        sd = s.std() or 1.0
...
def html_table(df: pd.DataFrame, title: str, subtitle: str) -> str:
    ...
    for i, r in df.iterrows():
        rows += (f"<tr><td class='rk'>{i+1}</td><td class='tk'>{r.ticker}</td>"
                 ...
                 f"<td>{r.volsurge:.1f}x</td><td>{r.above200:+.1%}</td></tr>")
...
<p>Generated {now} &middot; Index 3m return {idx_ret:+.1%} &middot; ranked by relative strength, momentum, volume</p></div>
